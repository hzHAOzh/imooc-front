<template>
  ...
  >
    <div>
      ...
      <m-svg-icon
        v-show="inputValue"
        class="w-1.5 h-1.5 absolute right-9 top-[50%] translate-y-[-50%] cursor-pointer duration-500"
        name="input-delete"
        @click="onClearClick"
      />
      <!-- 分割线 -->
      <div
        class="opacity-0 h-1.5 w-[1px] absolute right-[62px] top-[50%] translate-y-[-50%] duration-500 bg-zinc-200 group-hover:opacity-100"
      ></div>
      <!-- 搜索按钮 -->
      <m-button
    ...
    <transition name="slide">
      ...
      <div
        v-if="$slots.dropdown"
        v-show="isFocus"
        class="max-h-[368px] w-full text-base overflow-auto bg-white dark:bg-zinc-800 absolute left-0 top-[56px] z-20 p-2 rounded border border-zinc-200 dark:border-zinc-600 duration-200 hover:shadow-2xl scrollbar-thin scrollbar-thumb-rounded scrollbar-thumb-transparent xl:scrollbar-thumb-zinc-200 xl:dark:scrollbar-thumb-zinc-900 scrollbar-track-transparent"
      >
        <slot name="dropdown" />
      </div>
    </transition>
